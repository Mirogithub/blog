...
jQuery(document).ready(function($) {
    ...
    if (widthWindow < 631) {
        $('.shop-content').removeClass('list');
    }
    if (widthWindow < 993) {
        $('.title-login .one-block').click(function(){
            if (!$(this).hasClass('active')){
                $('.title-login .one-block, .box-tab').removeClass('active');
    ...
    $('.view-opt .v-1').click(function(e){
        e.preventDefault();
        var productList = $(this).closest('.shop-content');
        if (!productList.hasClass('list')){
            productList.addClass('list');
            $('.view-opt a').removeClass('active');
            $(this).addClass('active');
        }
    });

    $('.view-opt .v-2').click(function(e){
        e.preventDefault();
        var productList = $(this).closest('.shop-content');
        if (productList.hasClass('list') === true){
    ...
    $(function(){
        $(function() {
		  ...
		  " грн. - " + $( "#range-slider" ).slider( "values", 1 ) + " грн.");
        });
    });
